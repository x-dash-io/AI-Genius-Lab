...
export async function POST(request: NextRequest) {
  try {
    ...
    const summary = {
      ...
      lessonsWithMissingContent: result.filter((lesson) => !lesson.allContentExists).length,
      totalContentItems: result.reduce((sum, lesson) => sum + lesson.contentCount, 0),
      missingContentItems: result.reduce((sum, lesson) => sum + lesson.missingContent.length, 0),
    };
