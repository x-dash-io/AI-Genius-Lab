            ...
            <div className="flex-shrink-0 border-b p-6">
              <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                <Link href={getHref("/dashboard")} className="flex items-center gap-2">
                  ...
                  <span className="font-display text-lg font-bold tracking-tight">AI GENIUS LAB</span>
                </Link>
              </motion.div>
            </div>
                  ...
                  return (
                    <motion.div key={item.href} whileHover={{ x: 4 }}>
                      ...
                      >
                        ...
                        {isCart && cartCount > 0 && (
                          <Badge variant="destructive" className="h-5 min-w-[20px] flex items-center justify-center rounded-full px-1.5 text-[10px] font-bold">
              ...
              {session?.user && (
                <div className="space-y-4">
                  <Link href={getHref("/profile")}>
                    <motion.div whileHover={{ scale: 1.02 }} className="flex items-center gap-3 p-3 rounded-lg hover:bg-accent/50 transition-all group">
                      <div className="relative">
                        <Avatar className="h-12 w-12 ring-2 ring-primary/20 group-hover:ring-primary/40">
                      ...
                      <div className="flex-1 overflow-hidden min-w-0">
                        <p className="truncate text-sm font-medium group-hover:text-primary">{session.user.name || "User"}</p>
            ...
            {mobileMenuOpen && (
              <>
                ...
                <motion.nav
                  ...
                  className="fixed left-0 top-0 bottom-0 z-50 w-72 max-w-[85vw] bg-card/95 backdrop-blur-md border-r shadow-2xl overflow-y-auto"
                >
                  <div className="border-b p-4 flex items-center justify-between sticky top-0 bg-card/95 z-10 h-16">
                  ...
                  <div className="px-3 py-4 space-y-1">
                    {navigation.map((item, index) => {
                      ...
                      const cartCount = cart?.itemCount || 0;
                      return (
                        <motion.div key={item.href} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.05 * index }}>
                          <Link
                            href={getHref(item.href)}
                            onClick={() => setMobileMenuOpen(false)}
                            className={cn(
                          ...
                          >
                            ...
                            <span className="flex-1">{item.name}</span>
                            {isCart && cartCount > 0 && (
                              <Badge variant="destructive" className="h-5 min-w-[20px] flex items-center justify-center rounded-full text-[10px]">
                                {cartCount > 9 ? "9+" : cartCount}
                              </Badge>
                            )}
                          </Link>
                  ...
                  {session?.user && (
                    <div className="border-t px-4 py-4 space-y-4 sticky bottom-0 bg-background/95">
                      <Link href={getHref("/profile")} onClick={() => setMobileMenuOpen(false)}>
                        <div className="flex items-center gap-3 p-3 rounded-lg hover:bg-accent/50 group">
                          <Avatar className="h-12 w-12">
                            <AvatarImage src={avatarUrl || undefined} />
                            <AvatarFallback>{(session.user.name?.[0] || "U").toUpperCase()}</AvatarFallback>
                          </Avatar>
                          <div className="flex-1 overflow-hidden min-w-0">
